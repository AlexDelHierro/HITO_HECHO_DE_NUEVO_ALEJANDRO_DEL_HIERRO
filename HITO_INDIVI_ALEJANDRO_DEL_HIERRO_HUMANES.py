...
lista_deseos={}
class Comprar:
    def __init__(self,nomp,agregar):
        self.nomp=nomp
        self.agregar=agregar

        # La funcion Producto nos permite seleccionar donde queremos almacenar nuestros productos
    def Producto(self):
        self.nomp = input('Elige el producto deseado(El nombre debe estar exacto): ')
        print('--------------------------------------------------------------')
        # ¿Agregar a la cesta o a la lista de desos?
        self.agregar = float(input(f'Quieres agregar "{self.nomp}" a la cesta (1) o a la lista de deseos (2): '))
        # si lo agregamos a la cesta
        if self.agregar== 1:
            # Almacena el valor en la cesta
            cesta[self.nomp] = ropa[self.nomp]
        ...
        # si lo agregamos a la lista de deseos
            ...
            lista_deseos[self.nomp] = ropa[self.nomp]
            ...
            case 3:
                ...
                if fact1.lower() == 'email':
                    ...
                    if fact1.lower() == 'sms':
                        ...
                        print('Opción erronea porfavor reinicie el programa')
                    print()
pais1=Pago(input('Introduce tu pais'))
pais1.pagar()